@php
    $headline = match ($event ?? '') {
        'submitted' => 'An overtime request was submitted',
        'step' => 'Overtime awaiting next approval',
        'approved' => 'Your overtime request was approved',
        'rejected' => 'Your overtime request was rejected',
        'cancelled' => 'An overtime request was cancelled',
                    ...
                    <tr>
                        <td style="padding:28px;">
                            ...
                            <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#4b5563;">
                                <strong>{{ $employeeName }}</strong> · {{ $kindLabel }}<br>
                                {{ $workDate }} · {{ $hours }} hour(s)
                                @if (! empty($stepLabel))
                                    <br>Current step: {{ $stepLabel }}
                                @endif
                            </p>
                            <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#4b5563;">
                            ...
                            @endif
                            <p style="margin:0;">
                                <a href="{{ $overtimeUrl }}" style="display:inline-block;background:#c8102e;color:#ffffff;text-decoration:none;padding:12px 18px;border-radius:10px;font-size:14px;font-weight:600;">Open Overtime module</a>
                            </p>
                        </td>
                    </tr>
                    <tr>
